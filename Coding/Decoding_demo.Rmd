...
for (i in 1:8) points(counts[4,i,]+runif(24, -0.5, 0.5), counts[7,i,]+runif(24, -0.5, 0.5), pch=i, col=rainbow(180)[ceiling(oris[i])+1])
legend('topright', leg=unique(oris), col=rainbow(180)[unique(ceiling(oris))+1], pch=1:8, bty='n')
dev.off()
...
The variance is typically higher than the mean - so the spike count distribution does not look like if it was coming from a Poisson distribution. Still, we conclude that the errors are not too large. We will now perform Bayesian decoding based on the activity of cell 7. We use 14 randomly selected trials for training and the rest 10 trials for test, and estimate $\lambda$ as the mean spike count for the training trials of a given orientation.
...
* How do ou write the posterior from Bayes rule?
* Calculate the parameters of your model based on some training data!
* Evaluate the log likelihood ($\theta$) on test data!
* Compare it with the appropriately chosen chance level!
